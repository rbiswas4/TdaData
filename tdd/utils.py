"""
A module for utilities needed in the dataset
...
def make_dataframe(array, names, index):
    

    odict = Odict(zip(names, array))
    return pd.DataFrame(odict, index=index)

def pack_symm_array(arr, names=None):
    """
    Pack the symmetric matrix arr into upper triangular form and store
    in a one dimensional array, and use the names of the varianles  to construct
    the covariance names. 
    ...
    if names is not None:
        ...
        names = list('_'.join(['cov', x, y]) for (x, y) in list(zip(names[k], names[l])))
    
    return r, names


def unpack_symm_array(arr):
    """
    Unpack the upper triangular elements in a one dimensional array into the original
    symmetric matrix

    Parameters
    ----------

    arr: `np.array` with shape (l, )
        Upper triangular packing of a symmetric array arr
    """
    l = len(arr)

    n = np.int((-1 + np.sqrt(1+ 8*l))/2.)
    k, l = np.triu_indices(n)
    ...
    # The diagonal has been added twice
    a -= np.diagflat(np.diag(a))/2.
