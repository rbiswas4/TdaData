"""
Module for utilities specific to SNCosmo
"""
import numpy as np
import pandas as pd
...
class ResChar(object):
    ...
    def pack_sncosmo_fit_summaries(self):


        return self.pack_fits(self.vparams.values, self.vparam_names, self.covariance.values,
                              self.covariance.columns.values)
        


    @staticmethod
    def pack_fits(params, param_names, covariance, cov_names, suffix='_fit'):
        """
        Parameters
        ----------
        params : `np.ndarray` shape=(int,)

        param_names : sequence of strings 

        covariance: `np.ndarray` with shape(num, num)
            covariance matrix

        cov_names : list of strings
            list of variables in order along the columns
        ...
        """
        c, n = cutils.pack_symm_array(np.asarray(covariance), names=cov_names)

        # convert to list if array
        n = list(param_names) + list(n)
        n = list(_ + suffix for _ in n)


        # For the floats, ensure they are arrays
        c = np.hstack((np.asarray(params), np.asarray(c)))

        return Odict(zip(n, c))
