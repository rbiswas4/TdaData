...
class SNANASims(object):
    ...
    def snanadatafile(snanafileroot, filetype='head', location='./'):
        ...
        suffix = '_HEAD.FITS.gz'
        if filetype.lower() == 'phot':
            suffix = '_PHOT.FITS.gz'
    ...
    def get_headData(headFile, coerce_inds2int=False):
        ...
        if isinstance(_head['SNID'][0], string_types):
            ...
            df = _head.to_pandas()
            df['SNID'] = list(v.decode() for v in  df.SNID.values)
        return  df.set_index('SNID')
    ...
    def get_photrows(self, row=None, snid=None):
        ...
        if row is not None:
            ...
            ptrs = self.headData.loc[snid][['PTROBS_MIN', 'PTROBS_MAX']]
        else:
    ...
    def get_photdata(self):
        """
        obtain the photometry for this file
        """
        lst = list(self.get_SNANA_photometry(snid=snid).lightCurve for snid in self.headData.index.values)
        for i, (df, snid) in enumerate(zip(lst, self.headData.index.values)):
            df['snid'] = snid
            lst[i] = df
        df = pd.concat(lst)
        df['SNR'] = df.flux/df.fluxer
        return df
